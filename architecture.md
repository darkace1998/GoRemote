...
5. **Protocol Host**
   - Loads built-in Go-native protocol modules compiled into the binary.
6. **Credential Host**
...
- `plugins/protocol-{ssh,sftp,telnet,rlogin,rawsocket,serial,tn5250,rdp,mosh,http,vnc}` — built-in protocols (all Go-native packages, compiled into binary)
...
### Built-in protocols
All supported protocols (`ssh`, `sftp`, `telnet`, `rlogin`, `rawsocket`, `serial`, `tn5250`, `rdp`, `mosh`, `http`, `vnc`) are implemented as Go packages under `plugins/protocol-*` and registered at compile time via the protocol host. No subprocess or IPC path exists for any of these. PowerShell remoting remains a planned Go-native module and is not registered while it lacks a native transport.
