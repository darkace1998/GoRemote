...
- [HIGH] `plugins/protocol-http/session.go:130` (G402) — `tls.Config{InsecureSkipVerify: !verifyTLS}` is reachable when the user disables TLS verification on an HTTP probe. _Annotated with `#nosec G402` because this is an explicit user opt-out._ _Remediation:_ keep the existing UI warning, ensure verifyTLS defaults to `true`, and surface the relaxed mode in connection telemetry.
...
- [MEDIUM] `plugins/credential-bitwarden/provider.go:29`, `plugins/credential-1password/runner.go:31`, `internal/platform/notifier_*.go` (G204) — `exec.Command(<variable>)` with user-influenced arguments. _Remediation:_ document the trust boundary in each remaining process-spawning module and pin the executable via an absolute path looked up once at startup with `exec.LookPath` validation.
...
- **Goroutines lacking context cancellation** — every `go func(){…}` reviewed in `internal/app/sessions.go`, `host/plugin/ipc`, and `plugins/protocol-*` is paired with an explicit `<-ctx.Done()` arm, a wait channel, or a tied `Close()` lifecycle. No leaks identified during this pass.
